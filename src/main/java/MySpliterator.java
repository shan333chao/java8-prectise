...
public class MySpliterator {
    ...
    public static void main(String[] args) {
        ...
        MySpliteratorTxt txt=new MySpliteratorTxt(data);
        Optional.ofNullable(txt.stream().count()).ifPresent(System.out::println);
     //   txt.stream().filter(p->!p.equals("")).forEach(System.out::println);
        txt.parellelStream().filter(p->!p.equals("")).forEach(System.out::println);
    }

    static  class  MySpliteratorTxt{
        public   final String [] data;

        public Stream<String> stream(){
            return StreamSupport.stream(new HSpliterator(),false);
        }

        public Stream<String> parellelStream(){
            return StreamSupport.stream(new HSpliterator(),true);
        }
        public MySpliteratorTxt(String txt) {
            Objects.requireNonNull(txt,"TXT is empty");
            this.data=txt.split("\n");


        }
        private class  HSpliterator implements Spliterator<String>{
            private  int start,end;
            public HSpliterator() {
            this.start=0;
            this.end=MySpliteratorTxt.this.data.length-1;
            }

            public HSpliterator(int start, int end) {
                this.start = start;
                this.end = end;
            }
            ...
            public boolean tryAdvance(Consumer<? super String> action) {
                ...
                if (start<=end){
                    action.accept(MySpliteratorTxt.this.data[start++]);
                    return true;
                }
                return false;
            }
            ...
            public Spliterator<String> trySplit() {
                ...
                int right =start+mid;
                start=start+mid+1;
                return new HSpliterator(left,right);
            }
